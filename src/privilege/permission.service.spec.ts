...
import { CreatePermissionDto } from './dto/create-privilege.dto';
import {
  Permission,
  PermissionCategory,
  PermissionName,
...
import { PermissionService } from './privilege.service';
...
describe('PermissionService', () => {
  ...
  it('Should create permission', async () => {
    ...
    const result = await permissionRepository
      .findOne({ name: expectedCreatedPermissionName })
      .exec();
  ...
  it('Should find view dashboard permission', async () => {
    ...
    const result = await permissionService.findOne(
      createdPermission._id as string,
    );
  ...
  it('Update should be effective', async () => {
    ...
    const inDatabaseCreatedPermission = await permissionService.create(
      createdPermission,
    );
    await permissionService.update(inDatabaseCreatedPermission._id as string, {
      name: PermissionName.VIEW_DASHBOARD,
    });
    const result = await permissionService.findOne(
      inDatabaseCreatedPermission._id as string,
    );
