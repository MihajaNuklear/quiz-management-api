...
export class AuthService {
   ...
   */
  private readonly externalApiCaller: ExternalApiCaller = new ExternalApiCaller(
    'Captcha google',
  );
   ...
   */
  async validateUser(login: string, pass: string): Promise<User | null | any> {
    const userByUsername = await this.userService.findByUserName(login);
    const userByEmail = await this.userService.findUserByEmail(login);
    const user = userByUsername != null ? userByUsername : userByEmail != null ? userByEmail : null
  ...
  async handleFailedConnection(user: User) {
    ...
    if (user.isActive) {
      ...
      return await this.userService.handleTooManyFailedConnection(user);
    }
    return await this.userService.handleTooManyFailedConnection(user);
  }
