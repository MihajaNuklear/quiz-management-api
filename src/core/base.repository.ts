...
import {
  parseFilter,
  parsedFilterToMongoQueryOperator,
} from './base-respository-mongo.constant';
...
export abstract class BaseRepository<T extends Document, U> {
   ...
   */
  update(id: string, item: Partial<U> | any): Promise<U | null> {
  ...
  updateOne(
    ...
    return this.model.updateOne(target, item, filter);
  }
  ...
  getByListCriteria(
    ...
    const { page, pageSize, search, sortBy, sortOrder, ...filter } =
      parseQueryParams(criteria);
  ...
  getCsvExportData(
    ...
    const { search, sortBy, sortOrder, columns, ...filter } =
      parseQueryParams(criteria);
    return this.model
      .aggregate([
        // Do lookup if there is any:
  ...
  getList(
    ...
    return this.model.aggregate(stages).then(([{ items, totalItems }]) => ({
      items,
      totalItems: totalItems[0]?.totalItems || 0,
    }));
  }
  ...
  getAutocompleteSuggestions(
    ...
    return this.model
      .aggregate(stages)
      .then((results) => formatSearchAutocompleteResult(results));
  }
  ...
  private buildFilterRelatedCriteria(filter: Filter | undefined): object {
    ...
    const filterCriteria = Object.fromEntries(
      Object.entries(parsedFilter).map(([key, value]) => [
        key,
        parsedFilterToMongoQueryOperator(value),
      ]),
    );
