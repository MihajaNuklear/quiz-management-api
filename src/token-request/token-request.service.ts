...
export class TokenRequestService {
  ...
  async isSomeRequestNewPasswordPending(email: string): Promise<boolean> {
    return (
      (await this.tokenRequestRepository.count({
        userMail: email,
  ...
  async requestToken(
    ...
    const tokenExpirationDelay =
      this.configuration.requestToken
        .requestTokenExpirationForForgottenPassword;
    const { email } = user;

    const existingToken: TokenRequest | undefined =
      await this.tokenRequestRepository
        .findOne({
          userMail: email,
          status: TokenRequestStatus.PENDING,
        })
    ...
    if (existingToken) {
      ...
      return evaluatedToken.status === TokenRequestStatus.EXPIRED
        ? this.requestToken(user, type)
        : evaluatedToken;
    }

    return this.tokenRequestRepository.create({
      expirationDate: DateTime.now()
        .plus({ minutes: tokenExpirationDelay })
        .toJSDate(),
  ...
  async markTokenAsUsed(token: string) {
    await this.tokenRequestRepository.model
      .findOneAndUpdate(
        { token },
        { status: TokenRequestStatus.USED, usageDate: new Date() },
      )
      .exec();
  }
  ...
  async markTokenAsExpired(token: string) {
    await this.tokenRequestRepository.model
      .findOneAndUpdate({ token }, { status: TokenRequestStatus.EXPIRED })
      .exec();
  }
  ...
  async evaluateTokenExpiration(
    ...
    const isExpirationDatePassed =
      DateTime.now() > DateTime.fromJSDate(tokenRequest.expirationDate);
  ...
  async checkTokenValidity(
    ...
    const evaluatedTokenRequest = await this.evaluateTokenExpiration(
      tokenRequest,
    );
