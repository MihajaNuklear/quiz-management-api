...
import { CreateRoleDto } from '../group/dto/create-group.dto';
import { Role, RoleType, roleSchema } from '../group/entities/group.entity';
import { RoleRepository } from '../group/group.repository';
import { RoleService } from '../group/group.service';
...
describe('UserService', () => {
  ...
  beforeAll(async () => {
    ...
    tokenRequestRepository = module.get<TokenRequestRepository>(
      TokenRequestRepository,
    );
    mailNotificationRepository = module.get<MailNotificationRepository>(
      MailNotificationRepository,
    );
  ...
  it('Should create user', async () => {
    ...
    const result = await userRepository
      .findOne({ fullName: toBeCreatedUser.fullName })
      .exec();
  ...
  it('Should return users for CSV export', async () => {
    ...
    expect(csvUsers[0]).not.toEqual(
      expect.objectContaining({ email: toBeCreatedUser.email }),
    );
  });
  ...
  it('User update should be effective', async () => {
    ...
    const result = await userService.findOne(
      inDatabaseCreatedUser._id as string,
    );
  ...
  it('Should return permission names by user id', async () => {
    const inDatabaseCreatedUser = await userService.create(toBeCreatedUser);
    const results = await userService.getPermissionNamesByUserId(
      inDatabaseCreatedUser._id as string,
    );
    for (let i = 0; i < results.length; i++) {
      expect(results[i]).toEqual(
        (toBeCreatedUser.role as Role).permissions[i].name,
      );
    }
  });
  ...
  it('Should be able to request password reset', async () => {
    ...
    const foundMailNotification: MailNotification =
      await mailNotificationRepository
  ...
  it('Should be able to request init password', async () => {
    const inDatabaseCreatedUser = await userService.create(toBeCreatedUser);
    const populatedUser = await userService.findOne(
      inDatabaseCreatedUser._id as string,
    );
  ...
  it('Should be able to reset password', async () => {
    ...
    await userService.requestInitPassword(populatedUser);
    const { token }: TokenRequest = await tokenRequestRepository
      .findOne({})
      .lean({ userMail: toBeCreatedUser.email })
      .exec();
  ...
  it('Should update last connection date', async () => {
    const inDatabaseCreatedUser = await userService.create(toBeCreatedUser);
    const lastYear = DateTime.fromJSDate(new Date())
      .minus({ year: 1 })
      .toJSDate();
    await userService.update(inDatabaseCreatedUser._id as string, {
      lastConnectionDate: lastYear,
    });
    await userService.updateLastConnectionDate(
      inDatabaseCreatedUser._id as string,
    );
    const result = await userService.findOne(
      inDatabaseCreatedUser._id as string,
    ...
    expect(
      DateTime.fromJSDate(result.lastConnectionDate) >
        DateTime.fromJSDate(lastYear),
    ).toEqual(true);
  });
  ...
  it('Should batch insert user', async () => {
    ...
    expect(result.successfulInsertionCount).toEqual(
      expected.successfulInsertionCount,
    );
    expect(result.alreadyAddedToDatabaseCount).toEqual(
      expected.alreadyAddedToDatabaseCount,
    );
    expect(result.importDurationInSeconds).toBeGreaterThanOrEqual(
      expected.importDurationInSeconds,
    );
  });
